...
program trapezoidal_integration
  ...
  do n = 10, 1000, 100
    ...
    result = 0.5d0 * (f(a) + f(b)) ! Fill in the results. a and b are the endpoints, f(x) is the function. Assume 0.5 as weight!
    ...
    exact = (1.0d0/3.0d0) - 2.0d0*cos(1.0d0) + 2.0d0 ! Exact value of the integral. Use the above equation to calculate the exact value.
    error = abs(result - exact) ! Calculate the error. Use the absolute value of the difference between the result and the exact value.
...
contains
  ...
  function f(x) result(y)
    ...
    y = x**2 + 2.0d0 * sin(x) ! Function definition. Use the equation f(x) = x^2 + 2*sin(x)
  end function f
